...
@section('content')
    <div class="max-w-7xl mx-auto space-y-6 animate-fadeIn">
        ...
        {{-- Header de Estado --}}
        <div
            class="flex flex-col md:flex-row justify-between items-start md:items-center bg-zinc-900/50 border border-zinc-800 p-6 rounded-3xl backdrop-blur-md">
            ...
            <div class="mt-4 md:mt-0 flex gap-3">
                <span
                    class="px-4 py-2 {{ $dominio->desplegado ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400' }} rounded-xl border border-current text-xs font-bold uppercase tracking-widest">
                    {{ $dominio->desplegado ? '● Activo' : '○ Pendiente' }}
                </span>
                <form action="{{ route('dominios-reintentar', $dominio->id) }}" method="POST">
                    @csrf
                    <button
                        class="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-white rounded-xl text-xs font-bold transition-all border border-zinc-700">
                        🔄 Re-desplegar
                    </button>
                </form>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="space-y-4">
                            ...
                            <div class="flex flex-col">
                                ...
                                <span
                                    class="text-zinc-300 text-xs font-mono bg-black/30 p-2 rounded-lg mt-1 border border-zinc-800">
                                    {{ $dominio->full_path }}
                                </span>
                            </div>
                            <div class="flex flex-col">
                ...
                <div class="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
                    <h3 class="text-zinc-400 text-xs font-bold uppercase tracking-widest mb-6">Variables de Entorno (.env)
                    </h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                        @foreach ($dominio->envs as $env)
                            <div
                                class="flex justify-between items-center bg-black/20 border border-zinc-800/50 p-3 rounded-xl group">
                                <span class="text-zinc-500 text-[11px] font-mono truncate mr-2">{{ $env->key }}</span>
                                <div class="flex items-center gap-2 overflow-hidden">
                                    <span
                                        class="text-emerald-500 text-[11px] font-mono truncate">"{{ Str::limit($env->value, 15) }}"</span>
                                    <button onclick="copyRaw('{{ $env->value }}')"
                                        class="opacity-0 group-hover:opacity-100 text-zinc-600 hover:text-white transition-all text-[10px]">
                                        📋
                                    </button>
                                </div>
                            </div>
                    ...
                    <div class="space-y-4 font-mono text-sm">
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <div class="flex gap-2">
                                <input type="password" readonly value="{{ $dominio->db_pass }}"
                                    class="bg-black/30 border-none text-sky-400 text-xs w-full rounded p-1" id="db-pass">
                ...
                <div class="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 relative">
                    ...
                    <div class="flex flex-col gap-3">
                        <div class="relative group">
                            <input type="password" readonly value="{{ $dominio->api_key }}" id="api-key"
                        ...
                        <p class="text-[10px] text-zinc-600 italic">Usa esta clave para autenticar peticiones externas a la
                            API v1.</p>
                    </div>
                </div>
