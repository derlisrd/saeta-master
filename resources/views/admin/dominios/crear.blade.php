            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex flex-col gap-2 md:col-span-2 bg-sky-500/5 p-4 rounded-xl border border-sky-500/10 mb-2">
                    ...
                    <select name="user_id" required
                        ...
                        <option value="" disabled {{ old('user_id') ? '' : 'selected' }}>Seleccione el cliente
                            responsable...</option>
                ...
                {{-- Selector: Repositorio / Stack (Añadir este bloque) --}}
                <div class="flex flex-col gap-2 md:col-span-2 bg-zinc-800/30 p-4 rounded-xl border border-zinc-700/50 mb-2">
                    <label class="text-zinc-400 text-xs font-bold uppercase tracking-wider ml-1 flex items-center gap-2">
                        <svg class="w-4 h-4 text-sky-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" />
                        </svg>
                        Stack Tecnológico (Repositorio Git)
                    </label>
                    <select name="repositorio_id" required
                        class="bg-zinc-800 border border-zinc-700 rounded-xl p-3 text-white focus:ring-2 focus:ring-sky-500/50 transition-all">
                        <option value="" disabled selected>Seleccione el código base a desplegar...</option>
                        @foreach ($repositorios as $repo)
                            <option value="{{ $repo->id }}"
                                {{ old('repositorio_id') == $repo->id ? 'selected' : '' }}>
                                {{ $repo->nombre }} ({{ strtoupper($repo->tipo) }}) — Rama: {{ $repo->branch }}
                            </option>
                        @endforeach
                    </select>
                </div>

                {{-- Select: Servidor Destino (VM) --}}
                <div class="flex flex-col gap-2 md:col-span-2 bg-zinc-800/30 p-4 rounded-xl border border-zinc-700/50">
                    <label class="text-zinc-400 text-xs font-bold uppercase tracking-wider ml-1 flex items-center gap-2">
                        <svg class="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-2-4h.01M17 16h.01" />
                        </svg>
                        Servidor de Despliegue (VM)
                    </label>
                    <select name="vm_id" required
                        class="bg-zinc-800 border border-zinc-700 rounded-xl p-3 text-white focus:ring-2 focus:ring-emerald-500/50 transition-all">
                        <option value="" disabled selected>Seleccione un servidor activo...</option>
                        @foreach ($vms as $vm)
                            <option value="{{ $vm->id }}">
                                {{ $vm->nombre }} — {{ $vm->ip }} ({{ $vm->usuario }})
                            </option>
                        @endforeach
                    </select>
                </div>
